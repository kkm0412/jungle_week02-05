...
def merge(arr, left, mid, right):
    ...
    i = 0
    j = 0
    # print(arr[left:right],left,mid,right,len(arr[left:right]),"만큼")
    ...
    # left_arr에 남은 원소가 있으면 복사
        for t in range(i ,len(left_arr)):
            arr[left+t+j] = left_arr[t]
    # right_arr에 남은 원소가 있으면 복사
    elif i == len(left_arr):
        for t in range(j ,len(right_arr)):
            arr[left+i+t] = right_arr[t]
